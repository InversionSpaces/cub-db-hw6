...
def _rows_per_page(payload: str) -> int:
    row_bytes: bytes = StorageRow.serialize((payload,))
...
class TestCreateTable:
    def test_create_and_scan(self, store: InMemoryStorage) -> None:
        store.create_table(TableName("t"), ("a", "b"))
        store.insert_row(TableName("t"), ("x", "y"))
        rows = list(store.scan_rows(TableName("t")))
        assert len(rows) == 1

    def test_duplicate_table_raises(self, store: InMemoryStorage) -> None:
        store.create_table(TableName("t"), ("a",))
        with pytest.raises(DuplicateTableError):
            store.create_table(TableName("t"), ("b",))

    def test_get_columns(self, store: InMemoryStorage) -> None:
        store.create_table(TableName("t"), ("x", "y", "z"))
        assert store.get_columns(TableName("t")) == ("x", "y", "z")
...
class TestInsertRow:
    ...
    def test_insert_multiple_rows(self, store: InMemoryStorage) -> None:
        store.create_table(TableName("t"), ("a",))
        store.insert_row(TableName("t"), ("1",))
        store.insert_row(TableName("t"), ("2",))
        store.insert_row(TableName("t"), ("3",))
        rows = list(store.scan_rows(TableName("t")))
        assert len(rows) == 3
    ...
    def test_insert_after_delete(self, store: InMemoryStorage) -> None:
        ...
        store.commit()
        store.insert_row(TableName("t"), ("y",))
        rows = list(store.scan_rows(TableName("t")))
        assert len(rows) == 1
        assert rows[0][1] == ("y",)
...
class TestMarkUpdateCommit:
    def test_update_and_commit(self, store: InMemoryStorage) -> None:
        ...
        store.insert_row(TableName("t"), ("x",))
        rid, _ = list(store.scan_rows(TableName("t")))[0]
        store.mark_update(TableName("t"), rid, ("z",))
    ...
    def test_multiple_updates_last_write_wins(self, store: InMemoryStorage) -> None:
        ...
        store.insert_row(TableName("t"), ("x",))
        rid, _ = list(store.scan_rows(TableName("t")))[0]
        store.mark_update(TableName("t"), rid, ("y",))
        store.mark_update(TableName("t"), rid, ("z",))
        store.commit()
        _, row = list(store.scan_rows(TableName("t")))[0]
        assert row == ("z",)
    ...
    def test_update_multiple_rows(self, store: InMemoryStorage) -> None:
        ...
        store.insert_row(TableName("t"), ("2",))
        rows = list(store.scan_rows(TableName("t")))
        store.mark_update(TableName("t"), rows[0][0], ("x",))
        store.mark_update(TableName("t"), rows[1][0], ("y",))
        store.commit()
        values = {r[1][0] for r in store.scan_rows(TableName("t"))}
        assert values == {"x", "y"}
    ...
    def test_update_missing_table(self, store: InMemoryStorage) -> None:
        with pytest.raises(TableNotFoundError):
            store.mark_update(TableName("missing"), (PageId(0), SlotId(0)), ("z",))
...
class TestCommitWithUpdatesAndDeletes:
    def test_update_and_delete_different_rows(self, store: InMemoryStorage) -> None:
        store.create_table(TableName("t"), ("a",))
        store.insert_row(TableName("t"), ("keep",))
        store.insert_row(TableName("t"), ("delete",))
        rows = list(store.scan_rows(TableName("t")))
        store.mark_update(TableName("t"), rows[0][0], ("updated",))
        store.mark_delete(TableName("t"), rows[1][0])
        store.commit()
        rows = list(store.scan_rows(TableName("t")))
        assert len(rows) == 1
...
class TestTableDefSerialization:
    def test_roundtrip_simple(self) -> None:
        data = TableMeta.serialize(TableName("users"), ("name", "age"), PageId(1))
    ...
    def test_roundtrip_single_col(self) -> None:
        data = TableMeta.serialize(TableName("t"), ("a",), PageId(0))
        meta = TableMeta.deserialize(data)
        assert meta.name == "t"
    ...
    def test_roundtrip_unicode_name(self) -> None:
        data = TableMeta.serialize(TableName("テーブル"), ("列1", "列2"), PageId(5))
        meta = TableMeta.deserialize(data)
...
class TestPageRoundtrip:
    ...
    def test_metadata_page_roundtrip(self) -> None:
        page = Page.empty(PAGE_TYPE_META)
        item = TableMeta.serialize(TableName("users"), ("name", "age"), PageId(1))
        page.add_item(item)
        data = page.serialize()
        restored = Page.deserialize(data, PageId(0))
...
class TestFileStorageCreateTable:
    ...
    def test_metadata_entry_too_large_raises(self, file_store: FileStorage) -> None:
        ...
        with pytest.raises(RowTooLargeError):
            file_store.create_table(TableName(name), ("c",))
            file_store.commit()

    def test_get_columns(self, file_store: FileStorage) -> None:
        file_store.create_table(TableName("t"), ("x", "y", "z"))
        assert file_store.get_columns(TableName("t")) == ("x", "y", "z")
...
class TestFileStorageInsertRow:
    ...
    def test_insert_multiple_rows(self, file_store: FileStorage) -> None:
        file_store.create_table(TableName("t"), ("a",))
        file_store.insert_row(TableName("t"), ("1",))
        file_store.insert_row(TableName("t"), ("2",))
        file_store.insert_row(TableName("t"), ("3",))
        file_store.commit()
        rows = list(file_store.scan_rows(TableName("t")))
        assert len(rows) == 3
        values = {r[1][0] for r in rows}
    ...
    def test_insert_row_too_large(self, file_store: FileStorage) -> None:
        ...
        with pytest.raises(RowTooLargeError):
            file_store.insert_row(TableName("t"), (payload,))
...
class TestFileStorageMarkUpdateCommit:
    def test_update_and_commit(self, file_store: FileStorage) -> None:
        ...
        file_store.mark_update(TableName("t"), rid, ("z",))
        file_store.commit()
    ...
    def test_update_row_too_large_raises(self, file_store: FileStorage) -> None:
        ...
        with pytest.raises(RowTooLargeError):
            file_store.mark_update(TableName("t"), rid, (payload,))
            file_store.commit()
...
class TestFileStorageMarkDeleteCommit:
    def test_delete_and_commit(self, file_store: FileStorage) -> None:
        file_store.create_table(TableName("t"), ("a",))
        file_store.insert_row(TableName("t"), ("x",))
        file_store.insert_row(TableName("t"), ("y",))
        file_store.commit()
...
class TestFileStoragePersistence:
    def test_persistence_round_trip(self, tmp_path: Path) -> None:
        ...
        store.create_table(TableName("users"), ("name", "age"))
        store.insert_row(TableName("users"), ("alice", "30"))
        store.insert_row(TableName("users"), ("bob", "25"))
        store.commit()
        store.close()
    ...
    def test_multi_table(self, tmp_path: Path) -> None:
        db_path = tmp_path / "test.db"
        store = FileStorage(db_path)
        store.create_table(TableName("users"), ("name",))
        store.create_table(TableName("posts"), ("title",))
        store.insert_row(TableName("users"), ("alice",))
        store.insert_row(TableName("posts"), ("hello",))
        store.commit()
        store.close()
        store2 = FileStorage(db_path)
    ...
    def test_unicode_values(self, tmp_path: Path) -> None:
        db_path = tmp_path / "test.db"
        store = FileStorage(db_path)
        store.create_table(TableName("t"), ("val",))
        store.insert_row(TableName("t"), ("日本語",))
        store.commit()
        store.close()
        store2 = FileStorage(db_path)
        rows = list(store2.scan_rows(TableName("t")))
    ...
    def test_long_strings(self, tmp_path: Path) -> None:
        ...
        store = FileStorage(db_path)
        store.create_table(TableName("t"), ("val",))
        store.insert_row(TableName("t"), ("x" * 500,))
        store.commit()
        store.close()
        store2 = FileStorage(db_path)
        rows = list(store2.scan_rows(TableName("t")))
    ...
    def test_metadata_page_overflow_persists(self, tmp_path: Path) -> None:
        ...
        for i in range(total_tables):
            store.create_table(TableName(f"t{i:04d}"), ("c",))
        store.insert_row(TableName("t0000"), ("first",))
        store.insert_row(TableName(f"t{total_tables - 1:04d}"), ("last",))
        store.commit()
        store.close()
        store2 = FileStorage(db_path)
        for i in range(total_tables):
            assert store2.table_exists(TableName(f"t{i:04d}"))
        assert list(store2.scan_rows(TableName("t0000")))[0][1] == ("first",)
        assert list(store2.scan_rows(TableName(f"t{total_tables - 1:04d}")))[0][1] == ("last",)
    ...
    def test_metadata_page_growth_after_reload(self, tmp_path: Path) -> None:
        ...
        store2 = FileStorage(db_path)
        store2.create_table(TableName(f"t{max_tables:04d}"), ("c",))
        store2.insert_row(TableName(f"t{max_tables:04d}"), ("grown",))
        store2.commit()
        store2.close()

        store3 = FileStorage(db_path)
        assert store3.table_exists(TableName(f"t{max_tables:04d}"))
        assert list(store3.scan_rows(TableName(f"t{max_tables:04d}")))[0][1] == ("grown",)
    ...
    def test_non_continuous_page_allocation(self, tmp_path: Path) -> None:
        ...
        store.create_table(TableName("table_a"), ("col",))
        for _ in range(rows_per_page):
            store.insert_row(TableName("table_a"), (payload,))
        store.commit()

        store.create_table(TableName("table_b"), ("col",))
        store.insert_row(TableName("table_b"), ("b_row",))
        store.commit()
...
class TestFileStorageCompaction:
    def test_compaction_delete_all(self, tmp_path: Path) -> None:
        ...
        for _ in range(total_rows):
            store.insert_row(TableName("t"), (payload,))
        store.commit()
    ...
    def test_compaction_multiple_tables_complex_patterns(self, tmp_path: Path) -> None:
        ...
        store.create_table(TableName("a"), ("val",))
        for _ in range(rows_per_page_a * 3 + 1):
            store.insert_row(TableName("a"), (payload_a,))
        store.create_table(TableName("b"), ("val",))
        for _ in range(rows_per_page_b * 2 + 2):
            store.insert_row(TableName("b"), (payload_b,))
        store.create_table(TableName("c"), ("val",))
        for _ in range(rows_per_page_c * 4 + 3):
            store.insert_row(TableName("c"), (payload_c,))
        store.commit()
    ...
    def test_compaction_interleaved_meta_and_data_pages(self, tmp_path: Path) -> None:
        ...
        store.create_table(TableName("early_data"), ("val",))
        for _ in range(rows_per_page * 3):
            store.insert_row(TableName("early_data"), (data_payload,))
        ...
        store.create_table(TableName("late_data"), ("val",))
        for _ in range(rows_per_page * 2):
            store.insert_row(TableName("late_data"), (data_payload,))

        for i in range(total_tables):
            store.insert_row(TableName(f"t{i:04d}"), (f"val{i}",))
        store.commit()
